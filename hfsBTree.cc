...
// void* HfsBTree::search(BTreeKey* key, size_t dataLength) {
// 	u_int8_t* rootNode = new u_int8_t[this->_nodeSize];
// 	this->readNode(this->_header->rootNode, rootNode);
...
// 	// byteswap record offsets
// 	// set the pointer to the start of the offsets (numRecords + 1 for the freespace pointer)
// 	u_int16_t* endPtr = (u_int16_t*)(buffer + this->_nodeSize);
// 	for (u_int16_t* address = endPtr - (descriptor->numRecords + 1); address < endPtr; address++) {
...
// void HfsBTree::initIndexNode(u_int8_t* buffer) {
// 	BTNodeDescriptor* descriptor = (BTNodeDescriptor*)buffer;
// 	u_int16_t* endPtr = (u_int16_t*)(buffer + this->_nodeSize);
